import{I,A,g}from"./index.js";import{C}from"./index-0gt3gf92.js";var v=[{name:"index.tsx",type:"file"},{name:"components",type:"folder"},{name:"utils.ts",type:"file"},{name:"styles.css",type:"file"},{name:"hooks",type:"folder"}],z='<svg class="size-4 text-yellow-600" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path d="M2 6a2 2 0 012-2h5l2 2h5a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V6z" /></svg>',E='<svg class="size-4 text-neutral-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>',y="absolute inset-x-1 h-8 rounded bg-blue-100 dark:bg-blue-900/40",M={damping:35,stiffness:500,type:"spring"};function N(l){let n=0,s=!0,o=!1,r;l.innerHTML=A`<div class="flex size-full flex-col items-center justify-center gap-4 p-4">
    ...
    >
      ...
      <div data-ref="list" class="relative p-1">
        <div data-ref="highlight" class="${y}" style="top: 4px"></div>
  ...
  </div>`;let{browser:u,hint:b,list:k,mode:c}=g(l),{highlight:a}=g(l);function f(){r?.stop(),r=void 0,a.style.transform=""}function i(e){if(e===n)return;if(n=e,!s){a.style.top=`${4+n*32}px`;return}let t=a.getBoundingClientRect().top;f(),a.style.top=`${4+n*32}px`;let d=t-a.getBoundingClientRect().top;if(d===0)return;let m=a;m.style.transform=`translateY(${d}px)`,r=C(0,1000,{...M,onUpdate:(L)=>{let h=d*(1-L/1000);m.style.transform=h===0?"":`translateY(${h}px)`}})}function w(e){s=e,f();let t=document.createElement("div");t.className=y,t.style.top=`${4+n*32}px`,a.replaceWith(t),a=t,c.textContent=s?"Animated":"Instant",c.className=s?"text-xs text-blue-600 dark:text-blue-400":"text-xs text-neutral-600 dark:text-neutral-400"}function p(e){o=e,b.textContent=o?"Use ↑↓ to navigate":"Click to focus"}function x(e){if(!o)return;if(e.key==="ArrowDown")e.preventDefault(),i(Math.min(n+1,v.length-1));else if(e.key==="ArrowUp")e.preventDefault(),i(Math.max(n-1,0));else if(e.shiftKey&&!e.repeat)w(!s)}return u.addEventListener("focusin",()=>p(!0)),u.addEventListener("focusout",()=>p(!1)),k.addEventListener("click",(e)=>{let t=e.target.closest("[data-index]");if(t)i(Number(t.dataset.index))}),window.addEventListener("keydown",x),{destroy(){window.removeEventListener("keydown",x),r?.stop()}}}export{N as mount};
