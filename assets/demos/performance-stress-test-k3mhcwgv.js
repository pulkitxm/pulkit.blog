import{i,I,E,A,g}from"./index.js";import{C}from"./index-0gt3gf92.js";var v=[["path",{d:"M5 12h14"}]];var h=[["path",{d:"M5 12h14"}],["path",{d:"M12 5v14"}]];var b="Add items and load to see how CSS GPU acceleration outperforms JS under stress.",T="Under load: CSS animations stay smooth, JS animations may stutter.";function U(u){let t=10,n=null,l=[],d=[];u.innerHTML=A`<div class="flex size-full flex-col items-center justify-center gap-4 p-4">
    ...
    <p data-ref="text" class="max-w-md text-center text-neutral-500 text-xs dark:text-neutral-400">${b}</p>
  </div>`;let{minus:m,plus:f,count:y,load:c,cssTrack:k,motionTrack:x,text:w}=g(u);function M(){while(l.length<t){let e=l.length,a=`${8+e%5*20}px`,s=document.createElement("div");s.className="absolute left-0 h-3 w-3 rounded-full bg-green-500",s.style.animation="move-css 2s ease-in-out infinite alternate",s.style.top=a,k.append(s),l.push(s);let r=document.createElement("div");r.className="absolute h-3 w-3 rounded-full bg-orange-500",r.style.top=a,x.append(r),C(r,{x:8},{duration:0});let S=C(r,{x:[8,Math.min(180,Math.max(8,x.clientWidth-20))]},{delay:e/t*2,duration:2,ease:"easeInOut",repeat:Number.POSITIVE_INFINITY,repeatType:"reverse"});d.push({element:r,controls:S})}while(l.length>t){l.pop().remove();let{element:e,controls:a}=d.pop();a.stop(),e.remove()}}function o(){M(),l.forEach((a,s)=>{a.style.animationDelay=`${s/t*2}s`}),y.textContent=`${t} items`,m.disabled=t<=5,f.disabled=t>=50;let e=n!==null;c.className=i({variant:e?"destructive":"secondary",size:"sm"}),c.textContent=e?"Stop Load":"Add Load",w.textContent=e?T:b}function p(){if(n)clearInterval(n),n=null}return m.addEventListener("click",()=>{t=Math.max(5,t-5),o()}),f.addEventListener("click",()=>{t=Math.min(50,t+5),o()}),c.addEventListener("click",()=>{if(n)p();else n=setInterval(()=>{let e=0;for(let a=0;a<3000000;a++)e+=Math.sqrt(a)*Math.sin(a);return e},16);o()}),o(),{destroy(){p();for(let{controls:e}of d)e.stop()}}}export{U as mount};
